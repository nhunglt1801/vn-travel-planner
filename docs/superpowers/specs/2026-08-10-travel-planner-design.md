...
- **Frontend:** React 19 + Vite + TypeScript, SPA. Styling bằng CSS thuần/CSS Modules dùng CSS variables ánh xạ trực tiếp từ token trong DESIGN.md (màu, spacing, radius, shadow, typography). Không dùng thư viện UI ngoài — design system đã đủ chi tiết. **Ngoại lệ duy nhất (quyết định 2026-08-12, cập nhật 2026-08-12 #2):** ô chọn ngày đi ở `InputScreen` dùng `DayPicker` từ thư viện `react-day-picker` (thay cho `<input type="date">` mặc định của trình duyệt), theo yêu cầu trực tiếp của người dùng — màu khi chọn ngày dùng đúng màu accent coral `#F75940` qua CSS variable `--rdp-accent-color` của thư viện (không cần theme-provider/context nào). **Lịch sử:** ban đầu chọn `react-rainbow-components`, nhưng thư viện đó tự kéo theo một bản React 16 cũ (qua dependency thật, không phải peerDependency, của `react-async-script-loader`) gây lỗi runtime "Invalid hook call" khi thực sự render trong app React 19 — phát hiện ở bước nối `App.tsx` (Task 9, Plan 1), không phải lỗi biên dịch nên không lộ ra ở bước kiểm tra `tsc`. Đã đổi sang `react-day-picker` (không kéo theo dependency React nào khác, types export đúng chuẩn `exports` map, tương thích React 19).
...
- **Chuỗi nguồn ảnh theo thứ tự ưu tiên, đảm bảo luôn có ảnh** (yêu cầu cứng: không để ô ảnh trống):
  1. **Wikipedia trước** — tìm trang theo `imageQuery`/tên địa điểm, lấy ảnh thumbnail/original của trang nếu có → đây là ảnh thật đúng địa danh, ưu tiên hàng đầu. **Cập nhật 2026-08-12 (sửa lỗi ảnh sai chủ đề phát hiện khi kiểm thử với API key thật):** `imageQuery` do AI sinh ra là slug tiếng Anh (vd `"an-bang-beach"`), tìm thẳng trên Wikipedia tiếng Việt bằng full-text search dễ khớp nhầm sang trang hoàn toàn không liên quan (vd ra ảnh cờ Myanmar, ảnh nhân vật lịch sử) vì search không hiểu đây là tên riêng địa danh, chỉ khớp từ khoá rời rạc. Quy trình mới, đã kiểm chứng bằng gọi API thật trước khi áp dụng:
     1. Loại bỏ các từ chung chung khỏi `imageQuery` (vd "beach", "island", "bay"...) trước khi tìm kiếm.
     2. Tìm trên **Wikipedia tiếng Anh** bằng `action=opensearch` (chính xác hơn full-text search cho việc tìm đúng tên trang theo tên riêng).
     3. **Kiểm tra độ liên quan**: chỉ chấp nhận kết quả nếu mọi từ khoá (đã loại từ chung chung) trong query đều xuất hiện trong tiêu đề trang tìm được — nếu không khớp, coi như không tìm thấy, không dùng kết quả đó.
     4. Với trang tiếng Anh đã qua kiểm tra liên quan, lấy Wikidata QID của trang (`prop=pageprops`, cần `redirects=1`), rồi lấy tên bài viết tương ứng trên **Wikipedia tiếng Việt** qua Wikidata sitelinks (`action=wbgetentities&props=sitelinks&sitefilter=viwiki`) — không dùng `langlinks` (API cũ, phần lớn không còn dữ liệu trên Wikipedia hiện đại).
     5. Dữ liệu hiển thị cuối cùng (ảnh, alt text) luôn lấy từ đúng trang **Wikipedia tiếng Việt** tìm được ở bước 4, qua REST API `page/summary` như cũ.
     6. Bất kỳ bước nào ở trên thất bại (không tìm thấy trang liên quan, không có liên kết Wikidata sang tiếng Việt, trang tiếng Việt không có ảnh...) → coi như Wikipedia thất bại, rơi xuống bước 2 (Unsplash) — không hiển thị nội dung tiếng Anh khi không tìm được bản tiếng Việt tương ứng.
